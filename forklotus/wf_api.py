...
class wf_api(object):
	...
	def get_deimos_info(self):
		path = "https://api.warframestat.us/{platform}/cambionCycle".format(platform=self.platform)
		response = session.get(path)
		if response.status_code != 200:
	...
	def get_weapon_info(self, weapon):
		path = "https://api.warframestat.us/weapons/search/{query}".format(query=weapon)
		response = session.get(path)
		if response.status_code != 200:
	...
	def get_warframe_info(self, warframe):
		path = "https://api.warframestat.us/warframes/search/{query}".format(query=warframe)
		response = session.get(path)
		if response.status_code != 200:
